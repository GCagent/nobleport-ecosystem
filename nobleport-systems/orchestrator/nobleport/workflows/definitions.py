...
WORKFLOWS: dict[str, WorkflowDef] = {w.name: w for w in (
    ...
    _wf("nano_infill_chain",
        "NoblePort nano-ecosystem chain for infill/ADU development "
        "(hypothetical until verified parcel/zoning/market evidence)",
        Step("site_select", "nano.site_selector",
             "Hyperlocal acquisition screen — SITE-NNN STAGED"),
        Step("feasibility", "nano.feasibility",
             "Cost/value model — Stephanie does not authorize acquisition"),
        Step("entitlement", "nano.entitlement",
             "Verification matrix — nothing labeled permitted without evidence"),
        Step("design", "nano.generative_design",
             "Three concepts: max yield, lower-cost, balanced fit"),
        Step("sequence", "nano.sequence_optimizer",
             "Preconstruction through CO/closeout with critical-path inputs"),
        Step("stress", "nano.stress_tester",
             "Cost, schedule, rate, absorption, permitting scenarios"),
        Step("draw", "nano.draw_manager",
             "Draft construction-loan draw — HUMAN APPROVAL REQUIRED"),
        Step("ops", "nano.property_ops",
             "Post-closeout maintenance, vendors, OpEx, recurring problems"),
        Step("orchestrate", "nano.orchestrator",
             "Connect the evidence graph from parcel through asset performance"),
        Step("harness", "nano.five_harness",
             "Five-harness control plane — overall STAGED until real-parcel verification")),
)}
