...
CATALOG: tuple[ModuleSpec, ...] = (
    ...
    # -- Nano ecosystem chain (STAGED — hypothetical / due-diligence only) ---
    _m("nano.site_selector", "nano", "stephanie", Status.STAGED, Risk.LOW,
       "Hyperlocal site selection: zoning/use, geometry, utilities, access, flood, comps, permitting complexity",
       "nano", "infill"),
    _m("nano.feasibility", "nano", "stephanie", Status.STAGED, Risk.MEDIUM,
       "Nano-feasibility model: cost stack, completed value, spread, overrun sensitivity — never authorizes acquisition",
       "nano", "infill"),
    _m("nano.entitlement", "nano", "permitstream", Status.STAGED, Risk.MEDIUM,
       "Entitlement verification matrix; nothing labeled permitted without source evidence",
       "nano", "infill"),
    _m("nano.generative_design", "nano", "stephanie", Status.STAGED, Risk.LOW,
       "Three design concepts: maximum yield, lower-cost construction, balanced yield/community fit",
       "nano", "infill"),
    _m("nano.sequence_optimizer", "nano", "stephanie", Status.STAGED, Risk.MEDIUM,
       "Construction sequence optimizer: preconstruction through CO/closeout with critical-path inputs",
       "nano", "infill"),
    _m("nano.draw_manager", "nano", "stephanie", Status.STAGED, Risk.HIGH,
       "Construction loan draw package: budget, COs, completed work, invoices, lien waivers, inspection evidence",
       "nano", "infill"),
    _m("nano.stress_tester", "nano", "stephanie", Status.STAGED, Risk.LOW,
       "Risk & resilience stress tester: cost, schedule, rate, absorption, permitting scenarios",
       "nano", "infill"),
    _m("nano.property_ops", "nano", "stephanie", Status.STAGED, Risk.MEDIUM,
       "Smart property management copilot: maintenance, vendors, operating cost, recurring-problem detection",
       "nano", "infill"),
    _m("nano.orchestrator", "nano", "stephanie", Status.STAGED, Risk.LOW,
       "Ecosystem orchestrator: parcel → entitlement → drawings → estimate → subs → schedule → inspections → draws → closeout → tenants → maintenance → performance",
       "nano", "infill"),
    _m("nano.five_harness", "nano", "stephanie", Status.STAGED, Risk.LOW,
       "Five-harness control plane: execution, governance, security/compliance, observability/evidence, testing/release",
       "nano", "infill"),
